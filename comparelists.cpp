...
float compare (map<string, map<string, int> >& town_dicts, map<int, map<string, map<wchar_t*, vector<wchar_t*> > > >& distance_dict, /*map<wchar_t[], map<wchar_t[], charpair[]> >& word_matches,*/ string first_town, string second_town) {
  ...
  for (it1=first_dict->begin(); it1 != first_dict->end(); it1++) {
	...
	if (second_frequency >= MIN_FREQ) {
	  ...
	  if (distance_dict.count(distance) == 0) {
	    map<string, map<wchar_t*, vector<wchar_t*> > > newmap;
...
void printlist (map<int, map<string, map<wchar_t*, vector<wchar_t*> > > >& distance_dict, float avg_min_dist, string first_town, string second_town, const char* outfile) {
  ofstream output (outfile);
  output << "Comparison: " << first_town << " and " << second_town << " (" << avg_min_dist << ")" << endl;
  map<int, map<string, map<wchar_t*, vector<wchar_t*> > > >::iterator it1;
  for (it1=distance_dict.begin(); it1 != distance_dict.end(); it1++) {
    if (it1->first <= MAX_DIST) {
      output << "\n" << it1->first << " changes:" << endl;
      map<string, map<wchar_t*, vector<wchar_t*> > > paths = it1->second;
      multimap<int, string> path_counts;
      map<string, map<wchar_t*, vector<wchar_t*> > >::iterator it2;
...
int main (int argc, char* argv[]) {
  ...
  map<int, map<string, map<wchar_t*, vector<wchar_t*> > > > distance_dict;
