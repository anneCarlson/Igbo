...
void find_cognates (map<Town, map<wstring, float> >& town_dicts, const char* out1, const char* out2) {
	...
	if (cognates[second_town][first_town][second_word] != second_word) {
	  ...
	  if (curr_rand <= prob*RAND_MAX) {
	    if (prev_state != SAME) {
	      ...
	      for (int i=0; i < town_enc_ct; i++) {
		if (i != first_town && i != second_town) {
		  // each set of cognates is an equivalence class, and we are replacing the previous cognate with the second word in the equivalence class of the first word
		  // if something was cognate with the second word, remove that cognate
		  wstring new_second_cog = cognates[second_town][i][second_word];
		  if (new_second_cog.length() > 0) {
		    if (potential_cognates[i][second_town][new_second_cog].size() == 0)
		      potential_cognates[i][second_town][new_second_cog] = find_potential_cognates (new_second_cog, town_dicts[town_decoding[second_town]]);
		    enc_change change = find_change (potential_cognates[i][second_town][new_second_cog], second_word);
		    remove_cognate (cognates[i][second_town], cognates[second_town][i], corr_counts[i][second_town], corr_counts[second_town][i], corr_totals[i][second_town], corr_totals[second_town][i], new_second_cog, second_word, change/512, change%512, true);
		  }
		  // if a word was cognate with the first word, make it cognate with the second word
		  wstring first_cog = cognates[first_town][i][first_word];
		  if (first_cog.length() > 0) {
		    if (potential_cognates[i][second_town][first_cog].size() == 0)
		      potential_cognates[i][second_town][first_cog] = find_potential_cognates (first_cog, town_dicts[town_decoding[second_town]]);
		    enc_change change = find_change (potential_cognates[i][second_town][first_cog], second_word);
		    /*wcout << first_word << endl;
		    wcout << first_cog << endl;
		    wcout << second_word << endl;
		    wcout << change << endl;*/
		    add_cognate (cognates[i][second_town], cognates[second_town][i], corr_counts[i][second_town], corr_counts[second_town][i], corr_totals[i][second_town], corr_totals[second_town][i], first_cog, second_word, change/512, change%512);
		  }
		  // if something was cognate with the word previously matched with the first word, remove that cognate
		  wstring old_second_cog = cognates[second_town][i][prev_cognate];
		  if (old_second_cog.length() > 0) {
		    if (potential_cognates[i][second_town][old_second_cog].size() == 0)
		      potential_cognates[i][second_town][old_second_cog] = find_potential_cognates (old_second_cog, town_dicts[town_decoding[second_town]]);
		    enc_change change = find_change (potential_cognates[i][second_town][old_second_cog], prev_cognate);
		    remove_cognate (cognates[i][second_town], cognates[second_town][i], corr_counts[i][second_town], corr_counts[second_town][i], corr_totals[i][second_town], corr_totals[second_town][i], old_second_cog, prev_cognate, change/512, change%512, true);
		  }
		}
	      }
	      if (prev_state == DIFF) {
