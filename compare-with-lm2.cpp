...
bool one_change (Word first, Word second, enc_change& change) {
  ...
  Word::iterator it3 = it1+1;
  Word::iterator it4 = it2+1;
  while (*it3 == *it4) {
    if (it3 == first.end() && it4 == second.end()) {
      change = *it1*256 + *it2;
      return true;
    }
    else {
...
bool calculate_pair (TownWords& first_dict, TownWords& second_dict, map<wstring, Word>& split_dict1, map<wstring, Word>& split_dict2, ChangeMatches& change_dict, wstring first_word, wstring second_word) {
  ...
  if (close) {
    ...
    wcout << first_word << "," << second_word << endl;
  }
  return close;
}
...
void find_environments (enc_change change, WordPairs pair_dict, multimap<pair<enc_char, enc_char>, vector<wstring> >& mods_made, TownWords& first_unigrams, TownBigrams& first_bigrams, TownWords& second_unigrams, TownBigrams& second_bigrams) {
  ...
  int changed = test_environment (env_counts, mods, unigram_copy, bigram_copy, second_unigrams, second_bigrams, first_char, second_char);
  wcout << changed << endl;
  ...
  mods.clear();
  changed = test_environment (env_counts, mods, unigram_copy, bigram_copy, first_unigrams, first_bigrams, second_char, first_char);
  wcout << changed << endl;
  if (changed > max_changed) {
    max_changed = changed;
    max_unigrams = unigram_copy;
...
void char_matches (TownWords& first_town, TownWords& second_town, ChangeMatches change_dict, const char* outfile) {
  ...
  for (it3=first_chars.begin(); it3 != first_chars.end(); it3++) {
	...
	enc_char first_char = it4->first / 256;
	if (first_char == *it3)
  ...
  for (it3=second_chars.begin(); it3 != second_chars.end(); it3++) {
    if (first_chars.count(*it3) == 0) {
      output << "\t\t" << WChar_to_UTF8 (decoding[*it3].c_str()) << endl;
      for (it4=change_dict.begin(); it4 != change_dict.end(); it4++) {
	enc_char second_char = it4->first % 256;
	if (second_char == *it3)
	  output << it4->second.size() << "\t" << WChar_to_UTF8 (represent_change(it4->first).c_str()) << endl;
      }
      output << "\n";
...
int main (int argc, char* argv[]) {
  ...
  //char_matches (town_dicts[argv[3]], town_dicts[argv[4]], change_dict, argv[5]);
