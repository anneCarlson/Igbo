...
int edit_distance (const wchar_t* first, const wchar_t* second, vector<vector<charpair> >& paths) {
  ...
  vector<vector<charpair> > path_matrix[first_length][second_length];
...
float compare (map<string, map<string, int> >& town_dicts, map<int, map<wchar_t*, vector<wordpair> > >& distance_dict, /*map<wchar_t[], map<wchar_t[], charpair[]> >& word_matches,*/ string first_town, string second_town) {
  ...
  for (it1=first_dict.begin(); it1 != first_dict.end(); it1++) {
	...
	if (second_frequency >= MIN_FREQ) {
	  wchar_t* first_word = UTF8_to_WChar (it1->first.c_str());
	  wchar_t* second_word = UTF8_to_WChar (it2->first.c_str());
	  wordpair curr_pair;
	  curr_pair.first = first_word;
	  curr_pair.second = second_word;
	  vector<vector<charpair> > paths;
	  int distance = edit_distance(first_word, second_word, paths);
	  if (distance < minimum_distance)
	    minimum_distance = distance;
	  if (distance_dict.count(distance) == 0) {
	    map<wchar_t*, vector<wordpair> > newmap;
	  ...
	  for (it3=paths.begin(); it3 != paths.end(); it3++) {
	    ...
	    for (it4=it3->begin(); it4 != it3->end(); it4++) {
	      wchar_t test[] = {it4->first, it4->second, L'\0'};
	    ...
	    if (distance_dict[distance].count(charstring) == 0) {
	      vector<wordpair> newvector;
	      distance_dict[distance][charstring] = newvector;
	    }
	    distance_dict[distance][charstring].push_back(curr_pair);
	  }
	}
      }
...
void printlist (map<int, map<wchar_t*, vector<wordpair> > >& distance_dict, float avg_min_dist, string first_town, string second_town, const char* outfile) {
  ofstream output (outfile);
  output << "Comparison: " << first_town << " and " << second_town << " (" << avg_min_dist << ")" << endl;
  map<int, map<wchar_t*, vector<wordpair> > >::iterator it1;
  for (it1=distance_dict.begin(); it1 != distance_dict.end(); it1++) {
    if (it1->first <= MAX_DIST) {
      output << "\n" << it1->first << " changes:" << endl;
      map<wchar_t*, vector<wordpair> > paths = it1->second;
      multimap<int, wchar_t*> path_counts;
      map<wchar_t*, vector<wordpair> >::iterator it2;
      for (it2=paths.begin(); it2 != paths.end(); it2++) {
	pair<int, wchar_t*> path_count_pair (paths.count(it2->first), it2->first);
	path_counts.insert (path_count_pair);
      }
      multimap<int, wchar_t*>::reverse_iterator it3;
      for (it3=path_counts.rbegin(); it3 != path_counts.rend(); it3++) {
	cout << it3->second << endl;
	string path = represent_path (it3->second, (it1->first*2)+1);
	cout << path << endl;
	output << "\t\t" << path << "\t(" << it3->first << ")" << endl;
	vector<wordpair>::iterator it4;
	for (it4=paths[it3->second].begin(); it4 != paths[it3->second].end(); it4++) {
	  string first_word = WChar_to_UTF8 (it4->first);
	  string second_word = WChar_to_UTF8 (it4->second);
	  cout << it4->first << endl;
	  cout << it4->second << endl;
	  if (true)
	    output << path << "\t" << first_word << ", " << second_word << endl;
	}
      }
    }
  }
...
int main (int argc, char* argv[]) {
  ...
  map<int, map<wchar_t*, vector<wordpair> > > distance_dict;
