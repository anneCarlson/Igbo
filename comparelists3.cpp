...
float compare (map<string, map<string, int> >& town_dicts, map<int, map<string, vector<wordpair> > >& distance_dict, /*map<wchar_t[], map<wchar_t[], charpair[]> >& word_matches,*/ string first_town, string second_town) {
  ...
  for (it1=first_dict->begin(); it1 != first_dict->end(); it1++) {
	...
	if (second_frequency >= MIN_FREQ) {
	  ...
	  if (distance_dict.count(distance) == 0) {
	    map<string, vector<wordpair> > newmap;
	  ...
	  for (it3=paths.begin(); it3 != paths.end(); it3++) {
	    ...
	    if (distance_dict[distance].count(path) == 0) {
	      ...
	      distance_dict[distance][path] = newvector;
	    }
	    distance_dict[distance][path].push_back(curr_pair);
	  }
	}
      }
...
void printlist (map<int, map<string, vector<wordpair> > >& distance_dict, float avg_min_dist, string first_town, string second_town, const char* outfile) {
  ofstream output (outfile);
  output << "Comparison: " << first_town << " and " << second_town << " (" << avg_min_dist << ")" << endl;
  map<int, map<string, vector<wordpair> > >::iterator it1;
  for (it1=distance_dict.begin(); it1 != distance_dict.end(); it1++) {
    if (it1->first <= MAX_DIST) {
      output << "\n" << it1->first << " changes:" << endl;
      map<string, vector<wordpair> > paths = it1->second;
      multimap<int, string> path_counts;
      map<string, vector<wordpair> >::iterator it2;
	...
	for (it4=paths[it3->second].begin(); it4 != paths[it3->second].end(); it4++) {
	  ...
	  if (true)
	    output << it3->second << "\t" << first_word << ", " << second_word << endl;
	}
      }
    }
  }
...
int main (int argc, char* argv[]) {
  ...
  map<int, map<string, vector<wordpair> > > distance_dict;
