...
string represent_path (wchar_t* path, int distance) {
  ...
  string::iterator it;
  representation << distance << ":" << endl;
  for (int x=0; x != distance*2; x+=2) {
    if (path[x] == NULL_CHAR) {
...
float compare (map<string, map<string, int> >& town_dicts, map<string, map<string, vector<string> > >& distance_dict, /*map<wchar_t[], map<wchar_t[], charpair[]> >& word_matches,*/ string first_town, string second_town) {
  ...
  for (it1=first_dict->begin(); it1 != first_dict->end(); it1++) {
	...
	if (second_frequency >= MIN_FREQ) {
	  ...
	  for (it3=paths->begin(); it3 != paths->end(); it3++) {
	    ...
	    charstring[y] = L'\0';
	    string path = represent_path (charstring, distance);
	    if (distance_dict.count(path) == 0) {
	      map<string, vector<string> >* newmap = new map<string, vector<string> >;
	      distance_dict[path] = *newmap;
	      delete newmap;
	    }
	    if (distance_dict[path].count(it1->first) == 0) {
	      vector<string>* newvector = new vector<string>;
	      distance_dict[path][it1->first] = *newvector;
	      delete newvector;
	    }
	    distance_dict[path][it1->first].push_back(it2->first);
	  }
	  delete paths;
	}
      }
...
void printlist (map<string, map<string, vector<string> > >& distance_dict, float avg_min_dist, string first_town, string second_town, const char* outfile) {
  ...
  for (int x=0; x <= MAX_DIST; x++) {
    output << "\n" << x << " changes:" << endl;
    map<string, map<string, vector<string> > > paths = distance_dict;
...
int main (int argc, char* argv[]) {
  ...
  map<string, map<string, vector<string> > > distance_dict;
