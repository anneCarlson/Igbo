...
void gather_bigrams (map<enc_town, map<wstring, float> >& town_dicts, const char* listfile) {
  ...
  while (lists.good()) {
	...
	if (total_word_counts[first_encoded] >= MIN_COUNT && total_word_counts[second_encoded] >= MIN_COUNT) {
	   map<enc_word, int>  test =town_bigrams[curr_town][first_encoded];
	  ...
	  ++town_bigrams[curr_town][first_encoded][second_encoded];
	  ...
	  ++reverse_bigrams[curr_town][second_encoded][first_encoded];
...
void find_cognates (map<enc_town, map<wstring, float> >& town_dicts, const char* out1, const char* out2, const char* initfile) {
  ...
  while (iter < ITER) {
	...
	for (it4=reverse_bigrams[curr_town][curr_word_enc].begin(); it4 != reverse_bigrams[curr_town][curr_word_enc].end(); it4++) {
	  ...
	  double old_bigram_prob = (class_bigrams[new_class][first_class] + (class_counts[new_class]/arf_total)*DELTA)/(class_firsts[first_class] + DELTA);
	  double new_bigram_prob = (class_bigrams[new_class][first_class] + it4->second + (class_counts[new_class]+town_dicts[curr_town][curr_word]/arf_total)*DELTA)/(class_firsts[first_class] + DELTA);
    ...
    for (int i=0; i < cog_class_ct; i++) {
      //if (class_probs[i] > 0) {
      /*add_counts (curr_town, curr_word_enc, curr_count, i);
